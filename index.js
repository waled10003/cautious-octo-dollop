...
client.on("ready", () => {
    ...
    client.user.setActivity("dst!help", { type: "STREAMING", url:"https://www.twitch.tv/captain_motchy"})
})
  ...
  if (message.guild) {
    ...
    if (cmd === "help") {
      const embed = new MessageEmbed()
        ...
        .setDescription(`This project has been created by .-G- to let users to make their own modmail bot on their server(s) for free. You can check the support server by clicking [here](https://discord.gg/SayXkj9V3F).`)
        .addFields(
          {
    ...
    try {
      ...
      if (!channel) {
        ...
        const embedDM = new MessageEmbed()
          ...
          .setDescription(`Your Modmail Ticket is now closed. feel free to come back again if you still need some help.
`)
          .setColor("GREEN");
...
client.on('messageCreate', async (message) => {
  ...
  if (message.channel.parentId === category.id) {
    ...
    if (message.content.toLowerCase().startsWith(`${config.prefix}stop`)) {
      return; // Because we won't show to the user's the command {prefix}stop.
    } else {
